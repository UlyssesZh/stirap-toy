...
'''
data = np.loadtxt('output-zoomed.csv', delimiter=',')
x = data[:,0].reshape(201,201) / (2*np.pi) / 1e6
y = data[:,1].reshape(201,201) / (2*np.pi) / 1e6
z = data[:,2].reshape(201,201)**2
...
data = np.load('main-zoomed.npy')
x, y = np.meshgrid(np.linspace(1.875-0.02, 1.875+0.02, 201), np.linspace(1.875-0.02, 1.875+0.02, 201))
x = x.T
y = y.T
