...
plot_dir = 'paper/sigma1-sigma2-sweep'
output_file = 'paper/sigma1-sigma2-sweep.csv'
output_plot = 'paper/sigma1-sigma2-sweep.pdf'
import os
...
if len(data) == 0:
	...
	sigma1_vals = list(data[:, 2])
	sigma2_vals = list(data[:, 3])
	n1_vals = list(data[:, 4])
	n2_vals = list(data[:, 5])
...
count = 0
for i1, sigma1 in enumerate(linspace(2e-4, 12e-4, 51)):
	for i2, sigma2 in enumerate(linspace(2e-4, 12e-4, 51)):
		count += 1
		if count <= len(data):
			continue
		...
		if i1%10 == 0 and i2%10 == 0:
			...
			plt.legend()
			plt.xlabel("Time (ms)")
			plt.title(r'$\sigma_1=\SI{%.3f}{ms},\sigma_2=\SI{%.3f}{ms}$' % (sigma1*1000, sigma2*1000))
		...
		with open(output_file, 'a') as f:
			f.write(f'{i1},{i2},{sigma1},{sigma2},{n1_val},{n2_val}\n')

x = array(sigma1_vals).reshape(51,51) *1000
y = array(sigma2_vals).reshape(51,51) *1000
