...
plot_dir = 'paper/alpha0-kappa-sweep-larger'
output_file = 'paper/alpha0-kappa-sweep-larger.csv'
output_plot = 'paper/alpha0-kappa-sweep-larger.pdf'
import os
...
for i1, alpha0 in enumerate(linspace(500, 3500, 51)):
	for i2, kappa in enumerate(linspace(0, 2e6*2*pi, 101)):
		count += 1
		if count <= len(data):
			continue
		...
		if i1%5 == 0 and i2%10 == 0:
			plt.figure()
			plt.plot(t*1000, output.expect[0], label='Mechanical 1 number expec')
			plt.plot(t*1000, output.expect[1], label='Mechanical 2 number expec')
			plt.plot(t*1000, alpha_n1, label="Pulse 1")
			plt.plot(t*1000, alpha_n2, label="Pulse 2")
			plt.ylim(0, 2)
			plt.xlabel("Time (ms)")
			plt.title(r'$\alpha_0=%.3f,\kappa/2\pi=\SI{%.3f}{kHz}$' % (alpha0, kappa*1e-3/(2*pi)))
...
x = array(alpha0_vals).reshape(51,101)
y = array(kappa_vals).reshape(51,101) *1e-3/(2*pi)
z = array(n2_vals).reshape(51,101)
