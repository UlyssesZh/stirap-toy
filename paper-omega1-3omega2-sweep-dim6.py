...
for i, omega1 in enumerate(linspace(0.02e6-1800, 0.02e6+1800, 401)*2*pi):
	...
	fig, ax = hinton(final.ptrace(2))
	ax.set_title(r'$\omega_1/2\pi=\SI{%.6f}{MHz},\omega_2/2\pi=\SI{%.6f}{MHz}$' % (omega1*1e-6/(2*pi), omega2*1e-6/(2*pi)))
	plt.savefig(f'{plot_dir}/hinton-{i}.pdf', transparent=True, format='pdf', bbox_inches='tight')
