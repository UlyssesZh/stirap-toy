...
for i1, omega1 in enumerate(linspace(1e6, 2e6, 51)*2*pi):
	for i2, omega2 in enumerate(linspace(1e6, 2e6, 51)*2*pi):
		...
		if i1%10 == 0 and i2%10 == 0:
			...
			plt.savefig(f'plot-break-sigma/{i1}-{i2}.pdf', transparent=True, format='pdf', bbox_inches='tight')

		with open('output-break-sigma.csv', 'a') as f:
