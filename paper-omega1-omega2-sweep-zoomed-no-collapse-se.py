...
from qutip import mesolve, fidelity, hinton, plot_wigner, ket2dm
...
plot_dir = 'paper/omega1-omega2-sweep-zoomed-no-collapse-se'
output_file = 'paper/omega1-omega2-sweep-zoomed-no-collapse-se.csv'
output_plot = 'paper/omega1-omega2-sweep-zoomed-no-collapse-se.pdf'
import os
...
count = 0
for i1, omega1 in enumerate(linspace(0, 0.1e6, 201)[1:]*2*pi):
	for i2, omega2 in enumerate(linspace(0, 0.1e6, 201)[1:]*2*pi):
		count += 1
		if count <= len(data):
			continue
		...
		output = mesolve(H, ket2dm(psi0), t, c_ops=[], e_ops=e_ops, options=options)
...
x = array(omega1_vals).reshape(200,200) *1e-6/(2*pi)
y = array(omega2_vals).reshape(200,200) *1e-6/(2*pi)
z = array(n2_vals).reshape(200,200)
